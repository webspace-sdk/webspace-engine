    ...
    stage('build') {
      steps {
        script {
          ...
          def terraServer = env.TERRA_SERVER
          def corsProxyServer = env.CORS_PROXY_SERVER
          def nonCorsProxyDomains = env.NON_CORS_PROXY_DOMAINS
          def slackURL = env.SLACK_URL

          def habCommand = "/bin/bash scripts/hab-build-and-push.sh \\\"${baseAssetsPath}\\\" \\\"${shortlinkDomain}\\\" \\\"${reticulumServer}\\\" \\\"${thumbnailServer}\\\" \\\"${terraServer}\\\" \\\"${corsProxyServer}\\\" \\\"${nonCorsProxyDomains}\\\" \\\"${targetS3Bucket}\\\" \\\"${sentryDsn}\\\" \\\"${gaTrackingId}\\\" \\\"${env.BUILD_NUMBER}\\\" \\\"${env.GIT_COMMIT}\\\""
          ...
          def jelVersion = "${major}.${minor}.${version}.${packageTimeVersion}"
          ...
          def text = (
            ...
            "<https://github.com/jel-app/jel/commit/$gitSha|$gitSha> ${jelVersion} " +
            "Jel: ```${gitSha} ${gitMessage}```\n" +
            "<${smokeURL}?required_version=${jelVersion}|Smoke Test> - to push:\n" +
            "`/jel jel deploy ${jelVersion} s3://${targetS3Bucket}`"
          )
