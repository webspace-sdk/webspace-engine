...
import {assign, findByJob, findByTouch, jobIsAssigned, touchIsAssigned, unassign} from "./touchscreen/assignments";
...
const getPlayerCamera = (() => {
  ...
  return function() {
    if (!playerCamera) {
      playerCamera = DOM_ROOT.getElementById("viewing-camera").components.camera.camera;
    }
...
export class AppAwareTouchscreenDevice {
  constructor() {
    ...
    ["touchstart", "touchend", "touchmove", "touchcancel"].map(x =>
      DOM_ROOT.querySelector("canvas").addEventListener(x, this.events.push.bind(this.events))
    );
  }
