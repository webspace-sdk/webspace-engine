...
import {getHubIdFromHistory, getHubIdFromUrl, getSpaceIdFromHistory, getSpaceIdFromUrl} from "./jel-url-utils";
import {waitForDOMContentLoaded} from "./async-utils";
import {getHubMetaFromDOM, META_TAG_PREFIX} from "./dom-utils";
import {fetchSVoxFromUrl, getUrlFromVoxId, getVoxIdFromUrl} from "./vox-utils";
...
export class VoxMetadataSource extends EventTarget {
  ...
  async getVoxMeta(voxId) {
    ...
    return {
      vox_id: voxId,
      url: voxUrl,
      name: svoxModel.name || null,
      scale: svoxModel.scale ? (svoxModel.scale.x + svoxModel.scale.y + svoxModel.scale.z) / 3.0 : 1.0,
      stack_axis: Math.max(0, STACK_AXIS_CSS_NAMES.indexOf(svoxModel.stack_axis)),
      stack_snap_position: svoxModel.stack_snap_position || false,
      stack_snap_scale: svoxModel.stack_snap_scale || false
    };
  }
}
