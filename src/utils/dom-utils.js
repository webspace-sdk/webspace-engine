...
export async function getHubMetaFromDOM() {
  ...
  return {
    ...
    cors_proxy_url: getStringFromMetaTags("networking.cors_proxy_url", "https://worker.webspaces.workers.dev"),
...
export function createNewWebspaceDocument(title, projectionType = "spatial") {
  const metaTags = projectionType !== "spatial" ? '<meta name="webspace.projection.type" content="flat" />' : "";
...
export const prettifyHtml = html => {
  ...
  return docToPrettifiedHtml(doc.documentElement);
};
