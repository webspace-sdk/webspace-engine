...
import { isCurrentLocationIndexWorld } from "../utils/url-utils";
...
const CreateFileObjectPopup = forwardRef(
  ({ setPopperElement, styles, attributes, onCreate, objectType, fileExtension, filePath }, ref) => {
    ...
    if (objectType === "hub" && isCurrentLocationIndexWorld()) {
      contents = (
        <>
          <Info>
          ...
          <Tip>
            <FormattedMessage id={`create-${objectType}${translationSuffix}-popup.is-index-tip`} />
          </Tip>
          <SmallActionButton
            ref={ref}
            onClick={() => {
          ...
          >
            <FormattedMessage id={`create-${objectType}${translationSuffix}-popup.is-index-close`} />
          </SmallActionButton>
        </>
      );
    } else {
