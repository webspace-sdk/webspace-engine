...
export async function createInWorldLogMessage({ name, type, body }) {
  ...
  DOM_ROOT.querySelector("a-scene").appendChild(entity);
...
export async function spawnChatMessage(body, from) {
  ...
  try {
    ...
    if (url.host) {
      DOM_ROOT.querySelector("a-scene").emit("add_media", body);
      return;
    }
  } catch (e) {
    // Ignore parse error
  ...
  DOM_ROOT.querySelector("a-scene").emit("add_media", new File([blob], "message.png", { type: "image/png" }));
}
